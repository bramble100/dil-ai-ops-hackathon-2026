...
- **Hub+sub splitting is not always the right fix for over-budget pages.** Before recommending a split, evaluate the page's structure and purpose:
  - **Split makes sense when:** sections are independently useful and readers would navigate to them directly; the page covers multiple loosely related sub-topics; sections don't need to reference each other frequently.
  - **Keep as-is when:** the page's value comes from its chronological or narrative arc (splitting would fragment the story); `TOPIC.md` explicitly designates the page type as comprehensive/multi-source documents; the page is a reference list (quotes, timelines, catalogs) that benefits from single-page searchability; sections build on each other and cross-referencing between sub-pages would be clumsy.
  - When pages are over budget but splitting isn't warranted, report them as informational ("X lines, over ~Y guideline — structure reviewed, keeping as-is because [reason]") rather than as action items.
- **Structural imbalance may indicate missing page types, not over-categorization.** When one folder has 50 pages and another has 2, check `TOPIC.md` for intent. The fix might be creating more pages in the sparse folder (e.g., more case studies or financials) rather than redistributing from the dense one. Flag for human review with a specific recommendation.
